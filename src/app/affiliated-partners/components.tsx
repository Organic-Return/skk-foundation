...
export async function enrichPartnerWithAgentData(partner: Partner): Promise<EnrichedPartner> {
  ...
  const photoUrl = partner.overridePhoto
    ? urlFor(partner.overridePhoto)?.width(400).height(400).url() || null
  ...
  return (
    <div className={cardClasses}>
      <div className="flex flex-col items-center text-center">
        {/* Photo */}
        <Link href={partnerUrl} className={`relative ${featured ? 'w-32 h-32 md:w-40 md:h-40' : 'w-24 h-24 md:w-28 md:h-28'} rounded-full overflow-hidden mb-6 bg-[#f0f0f0] dark:bg-gray-800 block hover:opacity-90 transition-opacity`}>
          {partner.photoUrl ? (
            <Image
