...
// Years of Experience is shared with /buy: the Sell Page value wins if set,
// otherwise the Buy Page value, so one edit keeps both pages in step.
const BUY_YEARS_QUERY = `*[_id == "buyPage"][0].yearsOfExperience`;
...
export default async function SellPage() {
  ...
  const faqs: Faq[] = data.faqs && data.faqs.length > 0 ? data.faqs : DEFAULT_SELL_FAQS;
  const faqTitle: string = data.faqTitle || "Frequently Asked Questions About Selling in Aspen";
  const processTitle: string = data.processTitle || DEFAULT_SELL_PROCESS.title;
  const processIntro: string | null = data.processIntro || DEFAULT_SELL_PROCESS.intro;
  const processSteps =
  ...
  // Stats band — identical setup to /buy: a CMS override wins; otherwise years
  // of experience plus the live, baseline-driven sales totals shared with
  // /sold. No placeholders on a live page.
  const totals = await getSalesTotals();
  const years: number | undefined =
    typeof data.yearsOfExperience === "number" ? data.yearsOfExperience
    : typeof buyYears === "number" ? buyYears : undefined;
  const stats =
  ...
  // Testimonials — identical setup to /buy: page-specific set wins, else the
  // Testimonials page (featured first), three shown, link to all of them.
